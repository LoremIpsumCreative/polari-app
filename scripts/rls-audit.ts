...
function record(area: string, check: string, pass: boolean, detail: string) {
  ...
  console.log(
    `  ${pass ? 'PASS' : 'FAIL'}  ${area} · ${check}${pass ? '' : `\n        ↳ ${detail}`}`,
  );
}
  ...
  try {
    ...
    if (wordId) {
      ...
      await B.client
        ...
        .insert({ user_id: B.id, word_id: wordId, mastery: 1 });
    }
    ...
    await expectNoWrite(
      'anon',
      'cannot insert favourites',
      anon.from('favourites').insert({ user_id: B.id, word_id: wordId }).select(),
    );
    await expectNoWrite(
      'anon',
      'cannot insert quiz_stats',
      anon.from('quiz_stats').insert({ user_id: B.id }).select(),
    );
    ...
    for (const t of [
      ...
      await expectNoRows(
        'cross-read',
        `A cannot read B's ${t}`,
        A.client.from(t).select('*').eq(col, B.id),
      );
    }
    ...
    await expectNoWrite(
      ...
      A.client.from('profiles').update({ display_name: 'pwned' }).eq('id', B.id).select(),
    );
    await expectNoWrite(
      'cross-write',
      "A cannot update B's streaks",
      A.client.from('user_streaks').update({ current_streak: 9999 }).eq('user_id', B.id).select(),
    );
    await expectNoWrite(
      'cross-write',
      "A cannot update B's quiz_stats",
      A.client.from('quiz_stats').update({ timed_best: 9999 }).eq('user_id', B.id).select(),
    );
    await expectNoWrite(
      'cross-write',
      "A cannot delete B's favourites",
      A.client.from('favourites').delete().eq('user_id', B.id).select(),
    );
    await expectNoWrite(
      'cross-write',
      "A cannot delete B's progress",
      A.client.from('user_word_progress').delete().eq('user_id', B.id).select(),
    );
    await expectNoWrite(
      'cross-write',
      "A cannot delete B's quiz_attempts",
      A.client.from('quiz_attempts').delete().eq('user_id', B.id).select(),
    );

    console.log('\n─── 4. Forged ownership (A writing rows owned by B) ───');
    await expectNoWrite(
      'forgery',
      'A cannot insert a favourite as B',
      A.client.from('favourites').insert({ user_id: B.id, word_id: wordId }).select(),
    );
    await expectNoWrite(
      'forgery',
      'A cannot insert quiz_attempts as B',
      A.client
        .from('quiz_attempts')
        .insert({ user_id: B.id, score: 1, total_questions: 1 })
        .select(),
    );
    await expectNoWrite(
      'forgery',
      'A cannot insert progress as B',
      A.client
        .from('user_word_progress')
        .insert({ user_id: B.id, word_id: wordId, mastery: 5 })
        .select(),
    );
    await expectNoWrite(
      'forgery',
      'A cannot insert quiz_stats as B',
      A.client.from('quiz_stats').insert({ user_id: B.id }).select(),
    );
    await expectNoWrite(
      'forgery',
      'A cannot re-parent own row to B',
      A.client.from('favourites').update({ user_id: B.id }).eq('user_id', A.id).select(),
    );
    ...
    const forged = await anon.from('feedback').insert({
      user_id: B.id,
      message: marker,
      contact_email: 'attacker@polari.invalid',
    });
    ...
    record(
      'feedback',
      'anon cannot forge user_id on feedback',
      !!forged.error || !forgedRow || forgedRow.user_id !== B.id,
      forged.error
        ? `blocked: ${forged.error.message}`
        : forgedRow
          ? `WROTE a row falsely attributed to another user (${forgedRow.user_id})`
          : 'no row written',
    );
    ...
    for (let i = 0; i < 22; i++) {
      const r = await A.client
        .from('feedback')
        .insert({ user_id: A.id, message: `${marker}-flood-${i}` });
      if (!r.error) accepted++;
    }
    record(
      'feedback',
      'feedback inserts are rate limited per account',
      accepted <= 20,
    ...
    record(
      'feedback',
      'feedback message length is bounded',
      !!huge.error,
    ...
    await expectNoRows(
      ...
      anon.from('feedback').select('*').limit(10),
    );
    await expectNoRows(
      'feedback',
      'A cannot read feedback',
      A.client.from('feedback').select('*').limit(10),
    );
    ...
    record(
      'rpc',
      'anon cannot run record_daily_engagement',
      !!r1.error,
      r1.error ? `blocked: ${r1.error.message}` : 'EXECUTED as anon',
    );
    const r2 = await anon.rpc('record_quiz_game', { p_mode: 'timed', p_score: 999 });
    record(
      'rpc',
      'anon cannot run record_quiz_game',
      !!r2.error,
      r2.error ? `blocked: ${r2.error.message}` : 'EXECUTED as anon',
    );
    const r3 = await anon.rpc('handle_new_user');
    record(
      'rpc',
      'anon cannot run handle_new_user',
      !!r3.error,
      r3.error ? `blocked: ${r3.error.message}` : 'EXECUTED as anon',
    );

    // A's own RPC call must only ever touch A.
    await A.client.rpc('record_quiz_game', { p_mode: 'timed', p_score: 7 });
    const { data: bStats } = await admin
      .from('quiz_stats')
      .select('timed_best')
      .eq('user_id', B.id)
      .maybeSingle();
    record(
      'rpc',
      "A's record_quiz_game did not alter B",
      (bStats as { timed_best: number } | null)?.timed_best === 42,
    ...
    record(
      ...
      !ownProfile.error && (ownProfile.data?.length ?? 0) === 1,
      ownProfile.error?.message ?? `${ownProfile.data?.length} row(s)`,
    );

    const ownName = await A.client
      .from('profiles')
      .update({ display_name: 'Audit' })
      .eq('id', A.id)
      .select();
    record(
      'positive',
      'A can update own profile',
      !ownName.error && (ownName.data?.length ?? 0) === 1,
      ownName.error?.message ?? `${ownName.data?.length} row(s)`,
    );

    const ownFav = await A.client
      .from('favourites')
      .insert({ user_id: A.id, word_id: wordId })
      .select();
    record(
      'positive',
      'A can favourite a word',
      !ownFav.error && (ownFav.data?.length ?? 0) === 1,
    ...
    record(
      'positive',
      'A can unfavourite',
      !unfav.error && (unfav.data?.length ?? 0) >= 1,
      unfav.error?.message ?? `${unfav.data?.length} row(s)`,
    );

    const ownAttempt = await A.client
      .from('quiz_attempts')
      .insert({ user_id: A.id, score: 5, total_questions: 10 })
      .select();
    record(
      'positive',
      'A can record a quiz attempt',
      !ownAttempt.error && (ownAttempt.data?.length ?? 0) === 1,
    ...
    record(
      'positive',
      'A can run record_daily_engagement',
      !engagement.error,
    ...
    record(
      'positive',
      'anon can still read the dictionary',
      !dictionary.error && (dictionary.data?.length ?? 0) > 0,
    ...
    record(
      'positive',
      'anon can still read collections',
      !collections.error,
    ...
    const anonFeedback = await anon
      .from('feedback')
      .insert({ user_id: null, message: `${marker}-legit-anon`, category: 'bug' });
    record(
      'positive',
      'anonymous feedback still accepted',
      !anonFeedback.error,
      anonFeedback.error?.message ?? 'ok',
    );

    const authFeedback = await B.client
      .from('feedback')
      .insert({ user_id: B.id, message: `${marker}-legit-auth`, category: 'general' });
    record(
      'positive',
      'signed-in feedback still accepted',
      !authFeedback.error,
      authFeedback.error?.message ?? 'ok',
    );
    ...
    record(
      'storage',
      'anon can list character art (app depends on it)',
    ...
    record(
      'storage',
      'anon cannot upload to characters',
      !!anonUp.error,
      anonUp.error?.message ?? 'UPLOAD ACCEPTED',
    );

    const authUp = await A.client.storage
      .from('characters')
      .upload(`rls-audit-auth-${stamp}.png`, png);
    record(
      'storage',
      'signed-in user cannot upload to characters',
      !!authUp.error,
    ...
    record(
      'storage',
      'anon can list character wallpapers',
    ...
    if (existing) {
      ...
      record(
        'storage',
        'anon cannot delete character art',
        blocked,
    ...
    record(
      'schema',
      'auth.users not exposed via REST',
      !!authProbe.error,
      authProbe.error ? `blocked: ${authProbe.error.message}` : 'READABLE',
    );
